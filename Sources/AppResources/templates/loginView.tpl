
<div class="row">
    <div class="col-8">
        <img src="assets/images/login_image.png" class="img-fluid" style="width:90%;" />
    </div>
    <div class="col-4 center-vertically">
        <div class="font-22 text-gray">Zaloguj się</div>
        <form method="post">
            <div class="form-group">
                <label for="email" class="text-gray font-13">Email</label>
                <input type="email" name="email" class="form-control" id="email" aria-describedby="emailHelp" placeholder="john@example.com">
            </div>
            <div class="form-group">
                <label for="password" class="text-gray font-13">Hasło</label>
                <input type="password" name="password" class="form-control" id="email" aria-describedby="emailHelp" placeholder="••••••••••">
            </div>
            <div><input type="submit" class="btn btn-purple w-100" value="Zaloguj" /></div>
        </form>
        <p class="text-center mt-3 text-gray font-13">Chcesz dołączyć? <a href="#">Stwórz konto</a></p>
    </div>
</div>
